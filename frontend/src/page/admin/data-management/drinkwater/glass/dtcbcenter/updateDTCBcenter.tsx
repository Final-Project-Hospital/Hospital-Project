...
import { UpdateOrCreateDTCB, DeleteDTCB } from '../../../../../../services/drinkwaterServices/glass/dtcb';
...
interface UpdateDTCBCentralFormProps {
    initialValues: any; // รับค่า before/after 2 record
    onSuccess?: () => void;
    onCancel: () => void;
}

const UpdateDTCBCentralForm: React.FC<UpdateDTCBCentralFormProps> = ({
    initialValues,
    onSuccess,
    onCancel
}) => {
    ...
    const handleFinish = async (values: any) => {
        try {
            ...
            if (values.beforeAfterTreatmentID === 3) {
                ...
                const res = await UpdateOrCreateDTCB(beforePayload);
                ...
                await UpdateOrCreateDTCB(afterPayload);

            } else if (values.beforeAfterTreatmentID === 1) {
                await UpdateOrCreateDTCB({
                    ...basePayload,
                    ID: initialValues[0]?.ID ?? null,
            ...
            console.error("Error updating DTCB:", error?.response?.data || error);
...
export default UpdateDTCBCentralForm;