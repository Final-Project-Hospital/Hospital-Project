...
import { DFCBcenterInterface } from '../../../../../../interface/Idrinkwater/glass/Idfcb';
...
import { GetfirstDFCB, createDFCB } from '../../../../../../services/drinkwaterServices/glass/dfcb';
...
const { Option } = Select;
const DFCBCentralForm: React.FC<Props> = ({ onCancel, onSuccess }) => {
    ...
    const GetfirstrowDFCB = async () => {
        try {
            const responfirstDFCB = await GetfirstDFCB();
            if (responfirstDFCB.status === 200) {
                const data = responfirstDFCB.data;
                ...
                message.error("ไม่สามารถดึงข้อมูลการนัดหมายได้ สถานะ: " + responfirstDFCB.status);
            }
        } catch (error) {
    ...
    const handleFinish = async (values: any) => {
        ...
        if (selectedTreatmentID === 3) {
            ...
            const payloadBefore: DFCBcenterInterface = {
            ...
            const payloadAfter: DFCBcenterInterface = {
                Date: combinedDateTime.toISOString(),
            ...
            const res1 = await createDFCB(payloadBefore);
            const res2 = await createDFCB(payloadAfter);

            if ((res1 as any)?.status === 201 && (res2 as any)?.status === 201) {
                messageApi.success('บันทึกข้อมูล FCB of Glass ก่อนและหลังบำบัดสำเร็จ');
                form.resetFields();
            ...
            const dfcbData: DFCBcenterInterface = {
                Date: combinedDateTime.toISOString(),
                Data: values.data,
                Note: values.note,
            ...
            const response = await createDFCB(dfcbData);
            console.log(dfcbData);
            if (response.status === 201) {
                messageApi.open({
                    type: 'success',
                    content: 'การบันทึกข้อมูล DFCB of Glass สำเร็จ',
                });
                form.resetFields();
                setIsOtherunitSelected(false);
                setUseCustomStandard(false);
                ...
                >
                    <div className="dfcb-form-group">
                        <Form.Item label="วันที่บันทึกข้อมูล" name="date">
                            <DatePicker defaultValue={dayjs()} format="DD/MM/YYYY" className="dfcb-full-width" />
                        </Form.Item>

                        <Form.Item label="เวลาที่บันทึกข้อมูล" name="time">
                            <TimePicker defaultValue={dayjs()} format={"HH:mm"} className="dfcb-full-width" />
                        </Form.Item>
                    </div>
                    ...
                    <div className="dfcb-form-group">
                        ...
                        <div className="dfcb-from-mini">
                            ...
                            <div style={{ position: 'relative', top: '-15px' }}>
                                ...
                                {standardType === 'range' && useCustomStandard && (
                                    <div className="dfcb-fornt-small">
                                        <Form.Item
                                            label="ค่าต่ำสุด (Min)"
                                            name="customMin"
                    ...
                    <Form.Item className="dfcb-form-actions" >
                        ...
                        <Button htmlType="reset" className="dfcb-reset" onClick={handleClear} >
                            รีเซ็ต
                        </Button>
                        <Button type="primary" htmlType="submit" className="dfcb-submit">
                            บันทึก
                        </Button>
                    </Form.Item>
                </Form>
...
export default DFCBCentralForm;
