...
import { UpdateOrCreateDFCB, DeleteDFCB } from '../../../../../../services/drinkwaterServices/glass/dfcb';
...
interface UpdateDFCBCentralFormProps {
    initialValues: any; // รับค่า before/after 2 record
    onSuccess?: () => void;
    onCancel: () => void;
}

const UpdateDFCBCentralForm: React.FC<UpdateDFCBCentralFormProps> = ({
    initialValues,
    onSuccess,
    onCancel
}) => {
    ...
    const handleFinish = async (values: any) => {
        try {
            ...
            if (values.beforeAfterTreatmentID === 3) {
                ...
                const res = await UpdateOrCreateDFCB(beforePayload);
                ...
                await UpdateOrCreateDFCB(afterPayload);

            } else if (values.beforeAfterTreatmentID === 1) {
                await UpdateOrCreateDFCB({
                    ...basePayload,
                    ID: initialValues[0]?.ID ?? null,
            ...
            console.error("Error updating DFCB:", error?.response?.data || error);
...
export default UpdateDFCBCentralForm;