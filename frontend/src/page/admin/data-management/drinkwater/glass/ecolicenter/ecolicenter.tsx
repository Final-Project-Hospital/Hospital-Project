...
import { ECOcenterInterface } from '../../../../../../interface/Idrinkwater/glass/Ieco';
...
import { GetfirstECO, createECO } from '../../../../../../services/drinkwaterServices/glass/eco';
...
const { Option } = Select;
const ECOCentralForm: React.FC<Props> = ({ onCancel, onSuccess }) => {
    ...
    const GetfirstrowECO = async () => {
        try {
            const responfirstECO = await GetfirstECO();
            if (responfirstECO.status === 200) {
                const data = responfirstECO.data;
                ...
                message.error("ไม่สามารถดึงข้อมูลการนัดหมายได้ สถานะ: " + responfirstECO.status);
            }
        } catch (error) {
    ...
    const handleFinish = async (values: any) => {
        ...
        if (selectedTreatmentID === 3) {
            ...
            const payloadBefore: ECOcenterInterface = {
            ...
            const payloadAfter: ECOcenterInterface = {
                Date: combinedDateTime.toISOString(),
            ...
            const res1 = await createECO(payloadBefore);
            const res2 = await createECO(payloadAfter);

            if ((res1 as any)?.status === 201 && (res2 as any)?.status === 201) {
                messageApi.success('บันทึกข้อมูล E Coli of Glass ก่อนและหลังบำบัดสำเร็จ');
                form.resetFields();
            ...
            const ecoData: ECOcenterInterface = {
                Date: combinedDateTime.toISOString(),
                Data: values.data,
                Note: values.note,
            ...
            const response = await createECO(ecoData);
            console.log(ecoData);
            if (response.status === 201) {
                messageApi.open({
                    type: 'success',
                    content: 'การบันทึกข้อมูล E Coli of Glass สำเร็จ',
                });
                form.resetFields();
                setIsOtherunitSelected(false);
                setUseCustomStandard(false);
