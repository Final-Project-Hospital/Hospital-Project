...
import { UpdateOrCreateDTCBtank, DeleteDTCBtank } from '../../../../../../services/drinkwaterServices/tank/dtcbT';
...
interface UpdateDTCBtankCentralFormProps {
    initialValues: any; // รับค่า before/after 2 record
    onSuccess?: () => void;
    onCancel: () => void;
}

const UpdateDTCBtankCentralForm: React.FC<UpdateDTCBtankCentralFormProps> = ({
    initialValues,
    onSuccess,
    onCancel
}) => {
    ...
    const handleFinish = async (values: any) => {
        try {
            ...
            if (values.beforeAfterTreatmentID === 3) {
                ...
                const res = await UpdateOrCreateDTCBtank(beforePayload);
                ...
                await UpdateOrCreateDTCBtank(afterPayload);

            } else if (values.beforeAfterTreatmentID === 1) {
                await UpdateOrCreateDTCBtank({
                    ...basePayload,
                    ID: initialValues[0]?.ID ?? null,
            ...
            console.error("Error updating DTCBtank:", error?.response?.data || error);
...
export default UpdateDTCBtankCentralForm;