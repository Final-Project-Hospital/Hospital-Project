...
import './dtcbcenterT.css';
import { DTCBtankcenterInterface } from '../../../../../../interface/Idrinkwater/tank/IdtcbT';
...
import { GetfirstDTCBtank, createDTCBtank } from '../../../../../../services/drinkwaterServices/tank/dtcbT';
...
const { Option } = Select;
const DTCBtankCentralForm: React.FC<Props> = ({ onCancel, onSuccess }) => {
    ...
    const GetfirstrowDTCBtank = async () => {
        try {
            ...
            if (responfirstDTCBtank.status === 200) {
                ...
                message.error("ไม่สามารถดึงข้อมูลการนัดหมายได้ สถานะ: " + responfirstDTCBtank.status);
            }
        } catch (error) {
    ...
    const handleFinish = async (values: any) => {
        ...
        if (selectedTreatmentID === 3) {
            ...
            const payloadBefore: DTCBtankcenterInterface = {
            ...
            const payloadAfter: DTCBtankcenterInterface = {
                Date: combinedDateTime.toISOString(),
            ...
            const res1 = await createDTCBtank(payloadBefore);
            const res2 = await createDTCBtank(payloadAfter);

            if ((res1 as any)?.status === 201 && (res2 as any)?.status === 201) {
                messageApi.success('บันทึกข้อมูล TCB of Tank ก่อนและหลังบำบัดสำเร็จ');
                form.resetFields();
            ...
            const dtcbData: DTCBtankcenterInterface = {
                Date: combinedDateTime.toISOString(),
                Data: values.data,
                Note: values.note,
            ...
            const response = await createDTCBtank(dtcbData);
            console.log(dtcbData);
            if (response.status === 201) {
                messageApi.open({
                    type: 'success',
                    content: 'การบันทึกข้อมูล DTCBtank of Tank สำเร็จ',
                });
                form.resetFields();
                setIsOtherunitSelected(false);
                setUseCustomStandard(false);
...
export default DTCBtankCentralForm;
