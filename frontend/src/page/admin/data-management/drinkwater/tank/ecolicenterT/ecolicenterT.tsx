...
import './ecolicenterT.css';
import { ECOtankcenterInterface } from '../../../../../../interface/Idrinkwater/tank/IecoT';
...
import { GetfirstECOtank, createECOtank } from '../../../../../../services/drinkwaterServices/tank/ecoT';
...
const { Option } = Select;
const ECOtankCentralForm: React.FC<Props> = ({ onCancel, onSuccess }) => {
    ...
    const renderCustomTreatmentLabel = (text: string) => {
        ...
        return (
            <>
                ค่า E Coli บริเวณบ่อพักน้ำทิ้ง{colored}เข้าระบบบำบัด
            </>
        );
    };
    ...
    const GetfirstrowECOtank = async () => {
        try {
            const responfirstECOtank = await GetfirstECOtank();
            if (responfirstECOtank.status === 200) {
                const data = responfirstECOtank.data;
                ...
                message.error("ไม่สามารถดึงข้อมูลการนัดหมายได้ สถานะ: " + responfirstECOtank.status);
            }
        } catch (error) {
    ...
    const handleFinish = async (values: any) => {
        ...
        if (selectedTreatmentID === 3) {
            ...
            const payloadBefore: ECOtankcenterInterface = {
            ...
            const payloadAfter: ECOtankcenterInterface = {
                Date: combinedDateTime.toISOString(),
            ...
            const res1 = await createECOtank(payloadBefore);
            const res2 = await createECOtank(payloadAfter);

            if ((res1 as any)?.status === 201 && (res2 as any)?.status === 201) {
                messageApi.success('บันทึกข้อมูล E Coli of Tank ก่อนและหลังบำบัดสำเร็จ');
                form.resetFields();
            ...
            const ecoData: ECOtankcenterInterface = {
                Date: combinedDateTime.toISOString(),
                Data: values.data,
                Note: values.note,
            ...
            const response = await createECOtank(ecoData);
            console.log(ecoData);
            if (response.status === 201) {
                messageApi.open({
                    type: 'success',
                    content: 'การบันทึกข้อมูล E Coli of Tank สำเร็จ',
                });
                form.resetFields();
                setIsOtherunitSelected(false);
                setUseCustomStandard(false);
        ...
        <div>
            ...
            <div className="eco-container">
                ...
                >
                    ...
                    <Form.Item className="eco-form-actions" >
                        <Button className="eco-cancel" htmlType="button" onClick={handleCancelClick} >
                            ยกเลิก
                        </Button>
                        <Button htmlType="reset" className="eco-reset" onClick={handleClear} >
                            รีเซ็ต
                        </Button>
                        <Button type="primary" htmlType="submit" className="eco-submit">
                            บันทึก
                        </Button>
                    </Form.Item>
                </Form>
...
export default ECOtankCentralForm;
