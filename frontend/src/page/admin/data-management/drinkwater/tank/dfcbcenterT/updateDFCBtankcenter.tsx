...
import { UpdateOrCreateDFCBtank, DeleteDFCBtank } from '../../../../../../services/drinkwaterServices/tank/dfcbT';
...
interface UpdateDFCBtankCentralFormProps {
    initialValues: any; // รับค่า before/after 2 record
    onSuccess?: () => void;
    onCancel: () => void;
}

const UpdateDFCBtankCentralForm: React.FC<UpdateDFCBtankCentralFormProps> = ({
    initialValues,
    onSuccess,
    onCancel
}) => {
    ...
    const handleFinish = async (values: any) => {
        try {
            ...
            if (values.beforeAfterTreatmentID === 3) {
                ...
                const res = await UpdateOrCreateDFCBtank(beforePayload);
                ...
                await UpdateOrCreateDFCBtank(afterPayload);

            } else if (values.beforeAfterTreatmentID === 1) {
                await UpdateOrCreateDFCBtank({
                    ...basePayload,
                    ID: initialValues[0]?.ID ?? null,
            ...
            console.error("Error updating DFCBtank:", error?.response?.data || error);
...
export default UpdateDFCBtankCentralForm;