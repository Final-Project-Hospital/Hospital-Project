...
const DFCBtankCentralForm: React.FC<Props> = ({ onCancel, onSuccess }) => {
    ...
    const GetfirstrowDFCBtank = async () => {
        try {
            const responfirstDFCBtank = await GetfirstDFCBtank();
            if (responfirstDFCBtank.status === 200) {
                const data = responfirstDFCBtank.data;
                ...
                message.error("ไม่สามารถดึงข้อมูลการนัดหมายได้ สถานะ: " + responfirstDFCBtank.status);
            }
        } catch (error) {
...
export default DFCBtankCentralForm;
