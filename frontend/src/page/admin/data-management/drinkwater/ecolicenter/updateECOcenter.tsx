...
import { UpdateOrCreateECO, DeleteECO } from '../../../../../services/drinkwaterServices/eco';
...
interface UpdateECOCentralFormProps {
    initialValues: any; // รับค่า before/after 2 record
    onSuccess?: () => void;
    onCancel: () => void;
}

const UpdateECOCentralForm: React.FC<UpdateECOCentralFormProps> = ({
    initialValues,
    onSuccess,
    onCancel
}) => {
    ...
    const handleFinish = async (values: any) => {
        try {
            ...
            if (values.beforeAfterTreatmentID === 3) {
                ...
                const res = await UpdateOrCreateECO(beforePayload);
                ...
                await UpdateOrCreateECO(afterPayload);

            } else if (values.beforeAfterTreatmentID === 1) {
                await UpdateOrCreateECO({
                    ...basePayload,
                    ID: initialValues[0]?.ID ?? null,
            ...
            console.error("Error updating ECO:", error?.response?.data || error);
...
export default UpdateECOCentralForm;