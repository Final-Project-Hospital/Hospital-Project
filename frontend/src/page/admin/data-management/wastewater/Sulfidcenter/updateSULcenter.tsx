...
import { UpdateOrCreateSUL, DeleteSUL } from '../../../../../services/wastewaterServices/sul';
...
interface UpdateSULCentralFormProps {
    initialValues: any; // รับค่า before/after 2 record
    onSuccess?: () => void;
    onCancel: () => void;
}

const UpdateSULCentralForm: React.FC<UpdateSULCentralFormProps> = ({
    initialValues,
    onSuccess,
    onCancel
}) => {
    ...
    const handleFinish = async (values: any) => {
        try {
            ...
            if (values.beforeAfterTreatmentID === 3) {
                ...
                const res = await UpdateOrCreateSUL(beforePayload);
                ...
                await UpdateOrCreateSUL(afterPayload);

            } else if (values.beforeAfterTreatmentID === 1) {
                await UpdateOrCreateSUL({
                    ...basePayload,
                    ID: initialValues[0]?.ID ?? null,
            ...
            console.error("Error updating SUL:", error?.response?.data || error);
...
export default UpdateSULCentralForm;