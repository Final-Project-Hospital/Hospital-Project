...
import { SULcenterInterface } from '../../../../../interface/Iwastewater/Isul';
...
import { GetfirstSUL, createSUL } from '../../../../../services/wastewaterServices/sul';
...
const { Option } = Select;
const SULCentralForm: React.FC<Props> = ({ onCancel, onSuccess }) => {
    ...
    const GetfirstrowSUL = async () => {
        try {
            ...
            if (responfirstSUL.status === 200) {
                ...
                message.error("ไม่สามารถดึงข้อมูลการนัดหมายได้ สถานะ: " + responfirstSUL.status);
            }
        } catch (error) {
    ...
    const handleFinish = async (values: any) => {
        ...
        if (selectedTreatmentID === 3) {
            ...
            const payloadBefore: SULcenterInterface = {
            ...
            const payloadAfter: SULcenterInterface = {
                Date: combinedDateTime.toISOString(),
            ...
            const sulData: SULcenterInterface = {
                Date: combinedDateTime.toISOString(),
                Data: values.data,
                Note: values.note,
            ...
            const response = await createSUL(sulData);
            console.log(sulData);
                ...
                >
                    <div className="sul-form-group">
                        <Form.Item label="วันที่บันทึกข้อมูล" name="date">
                            <DatePicker defaultValue={dayjs()} format="DD/MM/YYYY" className="sul-full-width" />
                        </Form.Item>

                        <Form.Item label="เวลาที่บันทึกข้อมูล" name="time">
                            <TimePicker defaultValue={dayjs()} format={"HH:mm"} className="sul-full-width" />
                        </Form.Item>
                    </div>

                    <div className="sul-form-group">
                        <div className="sul-from-mini">
                            <Form.Item
                                label="หน่วยที่วัด"
                                required
                            >
                    ...
                    <Form.Item className="sul-form-actions" >
                        <Button className="sul-cancel" htmlType="button" onClick={handleCancelClick} >
                            ยกเลิก
                        </Button>
                        <Button htmlType="reset" className="sul-reset" onClick={handleClear} >
                            รีเซ็ต
                        </Button>
