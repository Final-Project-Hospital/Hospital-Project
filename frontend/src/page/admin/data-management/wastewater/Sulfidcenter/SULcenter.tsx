...
import { CreateSUL, GetfirstSUL } from "../../../../../services/enviromentrecord";
import { ListBeforeAfterTreatmentInterface } from "../../../../../interface/IBeforeAfterTreatment";
import {
  ListMiddleStandardInterface,
  ListRangeStandardInterface,
} from "../../../../../interface/IStandard";
import { ListUnitInterface } from "../../../../../interface/IUnit";
import { CreateSulfidInterface } from "../../../../../interface/IEnvironmentalRecord";

const { Option } = Select;

const SULCenterForm: React.FC = () => {
  //from
  const [form] = Form.useForm();

  const [beforeAfterOptions, setBeforeAfterOptions] = useState<
    ListBeforeAfterTreatmentInterface[]
  >([]);
  ...
  //standdart
  const [midddleStandards, setMiddleStandards] = useState<
    ListMiddleStandardInterface[]
  >([]);
  const [rangeStandards, setRangeStandards] = useState<
    ListRangeStandardInterface[]
  >([]);
  ...
  const [customSingleValue, setCustomSingleValue] = useState<
    number | undefined
  >(undefined);
  const [customMinValue, setCustomMinValue] = useState<number | undefined>(
    undefined
  );
  const [customMaxValue, setCustomMaxValue] = useState<number | undefined>(
    undefined
  );
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      const [beforeAfter, units, standardsMiddle, standardsRange] =
        await Promise.all([
          ListBeforeAfterTreatment(),
          ListUnit(),
          ListMiddleStandard(),
    ...
    const GetfirstrowSUL = async () => {
      try {
        const responfirstSUL = await GetfirstSUL();
        if (responfirstSUL.status === 200) {
          const data = responfirstSUL.data;
          ...
          message.error("ไม่สามารถดึงข้อมูล SUL ล่าสุดได้");
        }
      } catch (error) {
        console.error("Error fetching first SUL:", error);
  ...
  const handleFinish = async (values: any) => {
    try {
      ...
      if (selectedTreatmentID === 3) {
        ...
        const payloadBefore: CreateSulfidInterface = {
        ...
        const payloadAfter: CreateSulfidInterface = {
          Date: combinedDate.toISOString(),
        ...
        const res1 = await CreateSUL(payloadBefore);
        const res2 = await CreateSUL(payloadAfter);

        if (
          (res1 as any)?.status === 201 &&
          (res2 as any)?.status === 201
        ) {
          messageApi.success("บันทึกข้อมูล SUL ก่อนและหลังบำบัดสำเร็จ");
          form.resetFields();
          setSelectedTreatmentID(null);
          setUseCustomStandard(false);
          setTimeout(() => {
        ...
        const payload: CreateSulfidInterface = {
          Date: combinedDate.toISOString(),
          Data: values.data,
        ...
        const res = await CreateSUL(payload);

        if ((res as any)?.status === 201) {
          messageApi.success("บันทึกข้อมูล SUL สำเร็จ");
          form.resetFields();
          setSelectedTreatmentID(null);
          setUseCustomStandard(false);
          setTimeout(() => {
      ...
      console.error("Error Creating SUL:", error);
    ...
    <div>
      ...
      <div className="sul-container">
        ...
        >
              ...
              <div style={{ position: "relative", top: "-15px" }}>
                ...
                {standardType === "middle" && !useCustomStandard && (
                  <Form.Item
                    ...
                    name="standardID"
                    rules={[
                      { required: true, message: "กรุณาเลือกค่าเดี่ยว" },
                    ]}
                  >
                ...
                {standardType === "middle" && useCustomStandard && (
                  ...
                  >
                    <InputNumber
                      ...
                      onChange={(value) =>
                        setCustomSingleValue(value ?? undefined)
                      }
                    />
                  </Form.Item>
                )}
                ...
                {standardType === "range" && !useCustomStandard && (
                  <Form.Item
                    ...
                    name="standardID"
                    rules={[
                      { required: true, message: "กรุณาเลือกช่วงมาตรฐาน" },
                    ]}
                  >
                  ...
                  <div className="sul-fornt-small">
                    ...
                    >
                      <InputNumber
                        ...
                        onChange={(value) =>
                          setCustomMinValue(value ?? undefined)
                        }
                      />
                    </Form.Item>
                    <Form.Item
                    ...
                    >
                      <InputNumber
                        ...
                        onChange={(value) =>
                          setCustomMaxValue(value ?? undefined)
                        }
                      />
                    </Form.Item>
                  </div>
            ...
            <div className="sul-form-mini">
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: "กรุณาเลือกสถานะก่อน / หลัง / ก่อนเเละหลังบำบัด",
                  },
                ]}
              >
                <Select
          ...
          <div className="sul-from-group">
            <Form.Item label="หมายเหตุ" name="note">
              <Input.TextArea rows={2} placeholder="กรอกหมายเหตุ (ถ้ามี)" />
            </Form.Item>
          </div>

          <Form.Item className="sul-form-actions">
            <Button className="sul-cancel" htmlType="button">
              ยกเลิก
            </Button>
            <Button htmlType="reset" className="sul-reset" onClick={handleCancel}>
              รีเซ็ต
            </Button>
            <Button type="primary" htmlType="submit" className="sul-submit">
              บันทึก
            </Button>
          </Form.Item>
        </Form>
