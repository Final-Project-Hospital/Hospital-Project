...
import { CreateTCB, GetfirstTCB } from "../../../../../services/enviromentrecord";
...
import { CreateTCBInterface } from "../../../../../interface/IEnvironmentalRecord";

const { Option } = Select;

const TCBCenterForm: React.FC = () => {
  //from
  ...
  useEffect(() => {
    ...
    const GetfirstrowTCB = async () => {
      try {
        const responfirstTCB = await GetfirstTCB();
        if (responfirstTCB.status === 200) {
          const data = responfirstTCB.data;
          ...
          setSelectedTreatmentID(data.BeforeAfterTreatmentID);
            ...
            message.error("ไม่สามารถดึงข้อมูล TCB ล่าสุดได้");
        }
      } catch (error) {
          console.error("Error fetching first TCB:", error);
  ...
  const handleFinish = async (values: any) => {
    try{
        ...
        if (selectedTreatmentID === 3) {
          ...
          const payloadBefore: CreateTCBInterface = {
          ...
          const payloadAfter: CreateTCBInterface = {
              Date: combinedDate.toISOString(),
          ...
          const payload: CreateTCBInterface = {
              Date: combinedDate.toISOString(),
              Data: values.data,
          ...
          if((res as any)?.status === 201) {
            messageApi.success('บันทึกข้อมูล TCB สำเร็จ');
            form.resetFields();
            setSelectedTreatmentID(null);
            setUseCustomStandard(false);
            setTimeout(() => {
      ...
      console.error('Error Creating TCB:', error);
        ...
        >
          <div className="tcb-form-group">
            <Form.Item label="วันที่บันทึกข้อมูล" name="date">
              <DatePicker defaultValue={dayjs()} format="DD/MM/YYYY" className="tcb-full-width" />
            </Form.Item>
            
            <Form.Item label="เวลาที่บันทึกข้อมูล" name="time">
              <TimePicker defaultValue={dayjs()} format={"HH:mm"} className="tcb-full-width" />
            </Form.Item>
          </div>

          <div className="tcb-form-group">
            <div className = "tcb-form-mini">
              <Form.Item 
                label="หน่วยที่วัด" 
                required
              >
          ...
          <Form.Item className="tcb-form-actions">
            
              <Button className="tcb-cancel" htmlType="button">
                ยกเลิก
              </Button>
              <Button htmlType="reset" className="tcb-reset" onClick={handleCancel}>
                รีเซ็ต
              </Button>
              <Button type="primary" htmlType="submit" className="tcb-submit">
                บันทึก
              </Button>
          </Form.Item>
        </Form>
