...
import { CreateTS, GetfirstTS } from "../../../../../services/enviromentrecord";
    ...
    const GetfirstrowTS = async () => {
      try {
        ...
        if (responfirstTS.status === 200) {
          ...
          console.error("Error fetching first TS:", error);
  ...
  const handleFinish = async (values: any) => {
    try{
        ...
        if (selectedTreatmentID === 3) {
          ...
          const payloadBefore: CreateTSInterface = {
          ...
          const payloadAfter: CreateTSInterface = {
              Date: combinedDate.toISOString(),
          ...
          const payload: CreateTSInterface = {
              Date: combinedDate.toISOString(),
              Data: values.data,
          ...
          const res = await CreateTS(payload);

          if((res as any)?.status === 201) {
            messageApi.success('บันทึกข้อมูล TS สำเร็จ');
            form.resetFields();
            setSelectedTreatmentID(null);
            setUseCustomStandard(false);
            setTimeout(() => {
      ...
      console.error('Error Creating TS:', error);
      ...
      <div className="ts-container">
        ...
        >
          ...
          <Form.Item className="ts-form-actions">
              ...
              <Button className="ts-cancel" htmlType="button">
                ยกเลิก
              </Button>
              <Button htmlType="reset" className="ts-reset" onClick={handleCancel}>
                รีเซ็ต
              </Button>
