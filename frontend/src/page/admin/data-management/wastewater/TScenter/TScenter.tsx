...
import './TScenter.css';
import { TScenterInterface } from '../../../../../interface/Iwastewater/Its';
...
import { GetfirstTS, createTS } from '../../../../../services/wastewaterServices/ts';
...
const { Option } = Select;
const TSCentralForm: React.FC<Props> = ({ onCancel, onSuccess }) => {
    ...
    const renderCustomTreatmentLabel = (text: string) => {
        ...
        return (
            <>
                ค่า TS บริเวณบ่อพักน้ำทิ้ง{colored}เข้าระบบบำบัด
            </>
        );
    };
    ...
    const GetfirstrowTS = async () => {
        try {
            const responfirstTS = await GetfirstTS();
            if (responfirstTS.status === 200) {
                const data = responfirstTS.data;
                ...
                message.error("ไม่สามารถดึงข้อมูลการนัดหมายได้ สถานะ: " + responfirstTS.status);
            }
        } catch (error) {
    ...
    const handleFinish = async (values: any) => {
        ...
        if (selectedTreatmentID === 3) {
            ...
            const payloadBefore: TScenterInterface = {
            ...
            const payloadAfter: TScenterInterface = {
                Date: combinedDateTime.toISOString(),
            ...
            const res1 = await createTS(payloadBefore);
            const res2 = await createTS(payloadAfter);

            if ((res1 as any)?.status === 201 && (res2 as any)?.status === 201) {
                messageApi.success('บันทึกข้อมูล TS ก่อนและหลังบำบัดสำเร็จ');
                form.resetFields();
            ...
            const tsData: TScenterInterface = {
                Date: combinedDateTime.toISOString(),
                Data: values.data,
                Note: values.note,
            ...
            if (response.status === 201) {
                messageApi.open({
                    type: 'success',
                    content: 'การบันทึกข้อมูล TS สำเร็จ',
                });
                form.resetFields();
                setIsOtherunitSelected(false);
                setUseCustomStandard(false);
        ...
        <div>
            ...
            <div className="ts-container">
                ...
                >
                    ...
                    <Form.Item className="ts-form-actions" >
                        <Button className="ts-cancel" htmlType="button" onClick={handleCancelClick} >
                            ยกเลิก
                        </Button>
                        <Button htmlType="reset" className="ts-reset" onClick={handleClear} >
                            รีเซ็ต
                        </Button>
                        <Button type="primary" htmlType="submit" className="ts-submit">
                            บันทึก
                        </Button>
                    </Form.Item>
                </Form>
