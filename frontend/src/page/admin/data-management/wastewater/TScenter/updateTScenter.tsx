...
import { UpdateOrCreateTS, DeleteTS } from '../../../../../services/wastewaterServices/ts';
...
interface UpdateTSCentralFormProps {
    initialValues: any; // รับค่า before/after 2 record
    onSuccess?: () => void;
    onCancel: () => void;
}

const UpdateTSCentralForm: React.FC<UpdateTSCentralFormProps> = ({
    initialValues,
    onSuccess,
    onCancel
}) => {
    ...
    const handleFinish = async (values: any) => {
        try {
            ...
            if (values.beforeAfterTreatmentID === 3) {
                ...
                const res = await UpdateOrCreateTS(beforePayload);
                ...
                await UpdateOrCreateTS(afterPayload);

            } else if (values.beforeAfterTreatmentID === 1) {
                await UpdateOrCreateTS({
                    ...basePayload,
                    ID: initialValues[0]?.ID ?? null,
            ...
            console.error("Error updating TS:", error?.response?.data || error);
...
export default UpdateTSCentralForm;