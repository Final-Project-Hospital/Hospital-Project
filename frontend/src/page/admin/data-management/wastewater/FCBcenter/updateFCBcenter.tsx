...
import { UpdateOrCreateFCB, DeleteFCB } from '../../../../../services/wastewaterServices/fcb';
...
interface UpdateFCBCentralFormProps {
    initialValues: any; // รับค่า before/after 2 record
    onSuccess?: () => void;
    onCancel: () => void;
}

const UpdateFCBCentralForm: React.FC<UpdateFCBCentralFormProps> = ({
    initialValues,
    onSuccess,
    onCancel
}) => {
    ...
    const handleFinish = async (values: any) => {
        try {
            ...
            if (values.beforeAfterTreatmentID === 3) {
                ...
                const res = await UpdateOrCreateFCB(beforePayload);
                ...
                await UpdateOrCreateFCB(afterPayload);

            } else if (values.beforeAfterTreatmentID === 1) {
                await UpdateOrCreateFCB({
                    ...basePayload,
                    ID: initialValues[0]?.ID ?? null,
            ...
            console.error("Error updating FCB:", error?.response?.data || error);
...
export default UpdateFCBCentralForm;