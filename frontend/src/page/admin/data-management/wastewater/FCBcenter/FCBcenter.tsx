...
import { CreateFCB, GetfirstFCB } from "../../../../../services/enviromentrecord";
...
import { CreateFCBInterface } from "../../../../../interface/IEnvironmentalRecord";

const { Option } = Select;

const FCBCenterForm: React.FC = () => {
  //from
  ...
  useEffect(() => {
    ...
    const GetfirstrowFCB = async () => {
      try {
        const responfirstFCB = await GetfirstFCB();
        if (responfirstFCB.status === 200) {
          const data = responfirstFCB.data;
          ...
          setSelectedTreatmentID(data.BeforeAfterTreatmentID);
            ...
            message.error("ไม่สามารถดึงข้อมูล FCB ล่าสุดได้");
        }
      } catch (error) {
          console.error("Error fetching first FCB:", error);
  ...
  const handleFinish = async (values: any) => {
    try{
        ...
        if (selectedTreatmentID === 3) {
          ...
          const payloadBefore: CreateFCBInterface = {
          ...
          const payloadAfter: CreateFCBInterface = {
              Date: combinedDate.toISOString(),
          ...
          const res1 = await CreateFCB(payloadBefore);
          const res2 = await CreateFCB(payloadAfter);

          if ((res1 as any)?.status === 201 && (res2 as any)?.status === 201) {
            messageApi.success('บันทึกข้อมูล FCB ก่อนและหลังบำบัดสำเร็จ')
            form.resetFields();
            setSelectedTreatmentID(null);
            setUseCustomStandard(false);
            setTimeout(() =>{
          ...
          const payload: CreateFCBInterface = {
              Date: combinedDate.toISOString(),
              Data: values.data,
          ...
          const res = await CreateFCB(payload);

          if((res as any)?.status === 201) {
            messageApi.success('บันทึกข้อมูล FCB สำเร็จ');
            form.resetFields();
            setSelectedTreatmentID(null);
            setUseCustomStandard(false);
            setTimeout(() => {
      ...
      console.error('Error Creating FCB:', error);
    ...
    <div>
      ...
      <div className="fcb-container">
        ...
        >
          ...
          <Form.Item className="fcb-form-actions">
            
              <Button className="fcb-cancel" htmlType="button">
                ยกเลิก
              </Button>
              <Button htmlType="reset" className="fcb-reset" onClick={handleCancel}>
                รีเซ็ต
              </Button>
              <Button type="primary" htmlType="submit" className="fcb-submit">
                บันทึก
              </Button>
          </Form.Item>
        </Form>
