...
import { CreateCOD, GetfirstCOD } from "../../../../../services/enviromentrecord";
...
const CODCenterForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const GetfirstrowCOD = async () => {
      try {
        const responfirstCOD = await GetfirstCOD();
        if (responfirstCOD.status === 200) {
          const data = responfirstCOD.data;
          ...
          setSelectedTreatmentID(data.BeforeAfterTreatmentID);
            ...
            message.error("ไม่สามารถดึงข้อมูล COD ล่าสุดได้");
        }
      } catch (error) {
          console.error("Error fetching first COD:", error);
  ...
  const handleFinish = async (values: any) => {
    try{
        ...
        if (selectedTreatmentID === 3) {
          ...
          const payload: CreateCODInterface = {
              Date: combinedDate.toISOString(),
              Data: values.data,
          ...
          const res = await CreateCOD(payload);

          if((res as any)?.status === 201) {
            messageApi.success('บันทึกข้อมูล COD สำเร็จ');
            form.resetFields();
            setSelectedTreatmentID(null);
            setUseCustomStandard(false);
            setTimeout(() => {
    ...
    <div>
      ...
      <div className="cod-container">
        ...
        >
          ...
          <Form.Item className="cod-form-actions">
            
              <Button className="cod-cancel" htmlType="button">
                ยกเลิก
              </Button>
              <Button htmlType="reset" className="cod-reset" onClick={handleCancel}>
                รีเซ็ต
              </Button>
              <Button type="primary" htmlType="submit" className="cod-submit">
                บันทึก
              </Button>
          </Form.Item>
        </Form>
