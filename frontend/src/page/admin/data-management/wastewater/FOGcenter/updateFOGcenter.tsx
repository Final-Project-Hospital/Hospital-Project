...
import { UpdateOrCreateFOG, DeleteFOG } from '../../../../../services/wastewaterServices/fog';
...
interface UpdateFOGCentralFormProps {
    initialValues: any; // รับค่า before/after 2 record
    onSuccess?: () => void;
    onCancel: () => void;
}

const UpdateFOGCentralForm: React.FC<UpdateFOGCentralFormProps> = ({
    initialValues,
    onSuccess,
    onCancel
}) => {
    ...
    const handleFinish = async (values: any) => {
        try {
            ...
            if (values.beforeAfterTreatmentID === 3) {
                ...
                const res = await UpdateOrCreateFOG(beforePayload);
                ...
                await UpdateOrCreateFOG(afterPayload);

            } else if (values.beforeAfterTreatmentID === 1) {
                await UpdateOrCreateFOG({
                    ...basePayload,
                    ID: initialValues[0]?.ID ?? null,
            ...
            console.error("Error updating FOG:", error?.response?.data || error);
...
export default UpdateFOGCentralForm;