...
import { TKNcenterInterface } from '../../../../../interface/Iwastewater/Itkn';
...
import { GetfirstTKN, createTKN } from '../../../../../services/wastewaterServices/tkn';
...
const { Option } = Select;
const TKNCentralForm: React.FC<Props> = ({ onCancel, onSuccess }) => {
    ...
    const renderCustomTreatmentLabel = (text: string) => {
        ...
        return (
            <>
                ค่า TKN บริเวณบ่อพักน้ำทิ้ง{colored}เข้าระบบบำบัด
            </>
        );
    };
    ...
    const GetfirstrowTKN = async () => {
        try {
            ...
            if (responfirstTKN.status === 200) {
                ...
                message.error("ไม่สามารถดึงข้อมูลการนัดหมายได้ สถานะ: " + responfirstTKN.status);
            }
        } catch (error) {
    ...
    const handleFinish = async (values: any) => {
        ...
        if (selectedTreatmentID === 3) {
            ...
            const payloadBefore: TKNcenterInterface = {
            ...
            const payloadAfter: TKNcenterInterface = {
                Date: combinedDateTime.toISOString(),
            ...
            const res1 = await createTKN(payloadBefore);
            const res2 = await createTKN(payloadAfter);
            ...
            const tknData: TKNcenterInterface = {
                Date: combinedDateTime.toISOString(),
                Data: values.data,
                Note: values.note,
            ...
            if (response.status === 201) {
                messageApi.open({
                    type: 'success',
                    content: 'การบันทึกข้อมูล TKN สำเร็จ',
                });
                form.resetFields();
                setIsOtherunitSelected(false);
                setUseCustomStandard(false);
            ...
            <div className="tkn-container">
                ...
                >
                    ...
                    <Form.Item className="tkn-form-actions" >
                        <Button className="tkn-cancel" htmlType="button" onClick={handleCancelClick} >
                            ยกเลิก
                        </Button>
                        <Button htmlType="reset" className="tkn-reset" onClick={handleClear} >
                            รีเซ็ต
                        </Button>
