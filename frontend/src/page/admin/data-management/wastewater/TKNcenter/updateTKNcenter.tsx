...
import { UpdateOrCreateTKN, DeleteTKN } from '../../../../../services/wastewaterServices/tkn';
...
interface UpdateTKNCentralFormProps {
    initialValues: any; // รับค่า before/after 2 record
    onSuccess?: () => void;
    onCancel: () => void;
}

const UpdateTKNCentralForm: React.FC<UpdateTKNCentralFormProps> = ({
    initialValues,
    onSuccess,
    onCancel
}) => {
    ...
    const handleFinish = async (values: any) => {
        try {
            ...
            if (values.beforeAfterTreatmentID === 3) {
                ...
                const res = await UpdateOrCreateTKN(beforePayload);
                ...
                await UpdateOrCreateTKN(afterPayload);

            } else if (values.beforeAfterTreatmentID === 1) {
                await UpdateOrCreateTKN({
                    ...basePayload,
                    ID: initialValues[0]?.ID ?? null,
            ...
            console.error("Error updating TKN:", error?.response?.data || error);
...
export default UpdateTKNCentralForm;