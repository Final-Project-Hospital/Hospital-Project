...
import { REScenterInterface } from '../../../../../interface/Iwastewater/Ires';
...
import { GetfirstRES, createRES } from '../../../../../services/wastewaterServices/res';
...
const { Option } = Select;
const RESCentralForm: React.FC<Props> = ({ onCancel, onSuccess }) => {
    ...
    const GetfirstrowRES = async () => {
        try {
            ...
            if (responfirstRES.status === 200) {
                ...
                message.error("ไม่สามารถดึงข้อมูลการนัดหมายได้ สถานะ: " + responfirstRES.status);
            }
        } catch (error) {
    ...
    const handleFinish = async (values: any) => {
        ...
        if (selectedTreatmentID === 3) {
            ...
            const payloadBefore: REScenterInterface = {
            ...
            const payloadAfter: REScenterInterface = {
                Date: combinedDateTime.toISOString(),
            ...
            if ((res1 as any)?.status === 201 && (res2 as any)?.status === 201) {
                messageApi.success('บันทึกข้อมูล Residule ก่อนและหลังบำบัดสำเร็จ');
                form.resetFields();
            ...
            const resData: REScenterInterface = {
                Date: combinedDateTime.toISOString(),
                Data: values.data,
                Note: values.note,
            ...
            const response = await createRES(resData);
            console.log(resData);
                ...
                >
                    <div className="res-form-group">
                        <Form.Item label="วันที่บันทึกข้อมูล" name="date">
                            <DatePicker defaultValue={dayjs()} format="DD/MM/YYYY" className="res-full-width" />
                        </Form.Item>

                        <Form.Item label="เวลาที่บันทึกข้อมูล" name="time">
                            <TimePicker defaultValue={dayjs()} format={"HH:mm"} className="res-full-width" />
                        </Form.Item>
                    </div>

                    <div className="res-form-group">
                        <div className="res-from-mini">
                            <Form.Item
                                label="หน่วยที่วัด"
                                required
                            >
                    ...
                    <Form.Item className="res-form-actions" >
                        <Button className="res-cancel" htmlType="button" onClick={handleCancelClick} >
                            ยกเลิก
                        </Button>
                        <Button htmlType="reset" className="res-reset" onClick={handleClear} >
                            รีเซ็ต
                        </Button>
