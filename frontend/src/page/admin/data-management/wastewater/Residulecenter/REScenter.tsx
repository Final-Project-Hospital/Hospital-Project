// เปลี่ยนชื่อไฟล์เป็น RESCenterForm.tsx หากต้องการให้สมบูรณ์
import React, { useEffect, useState } from "react";
import dayjs from "dayjs";
import "./REScenter.css";
import {
  Form,
  InputNumber,
  Button,
...
import { CreateRES, GetfirstRES } from "../../../../../services/enviromentrecord";
...
import { CreateRESInterface } from "../../../../../interface/IEnvironmentalRecord";

const { Option } = Select;

const RESCenterForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const GetfirstrowRES = async () => {
      try {
        const responfirstRES = await GetfirstRES();
        if (responfirstRES.status === 200) {
          const data = responfirstRES.data;
          ...
          setSelectedTreatmentID(data.BeforeAfterTreatmentID);
            ...
            message.error("ไม่สามารถดึงข้อมูล RES ล่าสุดได้");
        }
      } catch (error) {
          console.error("Error fetching first RES:", error);
  ...
  const handleFinish = async (values: any) => {
    try {
      ...
      if (selectedTreatmentID === 3) {
        const payloadBefore: CreateRESInterface = {
        ...
        const payloadAfter: CreateRESInterface = {
            Date: combinedDate.toISOString(),
        ...
        const res1 = await CreateRES(payloadBefore);
        const res2 = await CreateRES(payloadAfter);

        if ((res1 as any)?.status === 201 && (res2 as any)?.status === 201) {
          messageApi.success('บันทึกข้อมูล RES ก่อนและหลังบำบัดสำเร็จ');
          form.resetFields();
          setSelectedTreatmentID(null);
          setUseCustomStandard(false);
        ...
        const payload: CreateRESInterface = {
            Date: combinedDate.toISOString(),
            Data: values.data,
        ...
        const res = await CreateRES(payload);

        if((res as any)?.status === 201) {
          messageApi.success('บันทึกข้อมูล RES สำเร็จ');
          form.resetFields();
          setSelectedTreatmentID(null);
          setUseCustomStandard(false);
          setTimeout(() => window.location.reload(),1500);
      ...
      console.error('Error Creating RES:', error);
      ...
      >
        <div className="res-form-group">
          <Form.Item label="วันที่บันทึกข้อมูล" name="date">
            <DatePicker defaultValue={dayjs()} format="DD/MM/YYYY" className="cod-full-width" />
          </Form.Item>
          
          <Form.Item label="เวลาที่บันทึกข้อมูล" name="time">
            <TimePicker defaultValue={dayjs()} format={"HH:mm"} className="cod-full-width" />
          </Form.Item>
        </div>

        <div className="res-form-group">
          <div className="res-form-mini">
        ...
        <Form.Item className="res-form-actions">
          <Button className="res-cancel" htmlType="button">
            ยกเลิก
          </Button>
          <Button htmlType="reset" className="res-reset" onClick={handleCancel}>
            รีเซ็ต
          </Button>
          <Button type="primary" htmlType="submit" className="res-submit">
            บันทึก
          </Button>
        </Form.Item>
      </Form>
