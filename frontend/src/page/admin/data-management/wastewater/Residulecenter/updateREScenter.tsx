...
import { UpdateOrCreateRES, DeleteRES } from '../../../../../services/wastewaterServices/res';
...
interface UpdateRESCentralFormProps {
    initialValues: any; // รับค่า before/after 2 record
    onSuccess?: () => void;
    onCancel: () => void;
}

const UpdateRESCentralForm: React.FC<UpdateRESCentralFormProps> = ({
    initialValues,
    onSuccess,
    onCancel
}) => {
    ...
    const handleFinish = async (values: any) => {
        try {
            ...
            if (values.beforeAfterTreatmentID === 3) {
                ...
                const res = await UpdateOrCreateRES(beforePayload);
                ...
                await UpdateOrCreateRES(afterPayload);

            } else if (values.beforeAfterTreatmentID === 1) {
                await UpdateOrCreateRES({
                    ...basePayload,
                    ID: initialValues[0]?.ID ?? null,
            ...
            console.error("Error updating RES:", error?.response?.data || error);
...
export default UpdateRESCentralForm;