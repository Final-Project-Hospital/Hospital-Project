...
import { HazardouscenterInterface } from '../../../../../interface/Igarbage/IhazardousWaste';
...
const HazardousWasteForm: React.FC<Props> = ({ onCancel, onSuccess }) => {
  ...
  const handleFinish = async (values: any) => {
    try {
      ...
      const response = await createHazardous(payload);
        ...
        >
          <div className="haz-form-group">
            <Form.Item label="วันที่บันทึกข้อมูล" name="date">
              <DatePicker defaultValue={dayjs()} format="DD/MM/YYYY" className="haz-full-width" />
            </Form.Item>

            <Form.Item label="เวลาที่บันทึกข้อมูล" name="time">
              <TimePicker defaultValue={dayjs()} format={"HH:mm"} className="haz-full-width" />
            </Form.Item>
          </div>
          ...
          <Form.Item className="haz-form-actions" >
            ...
            <Button htmlType="reset" className="haz-reset" onClick={handleClear} >
              รีเซ็ต
            </Button>
            <Button type="primary" htmlType="submit" className="haz-submit">
              บันทึก
            </Button>
          </Form.Item>
        </Form>
