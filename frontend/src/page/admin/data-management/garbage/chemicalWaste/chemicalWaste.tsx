...
import './chemicalWaste.css';
import { ChemicalcenterInterface } from '../../../../../interface/Igarbage/IchemicalWaste';
import { createChemical, GetfirstChemical } from '../../../../../services/garbageServices/chemicalWaste';
...
const { Option } = Select;
const ChemicalWasteForm: React.FC<Props> = ({ onCancel, onSuccess }) => {
  ...
  const GetfirstrowChemical = async () => {
    try {
      const responfirstHAS = await GetfirstChemical();
  ...
  const handleFinish = async (values: any) => {
    try {
      ...
      const payload: ChemicalcenterInterface = {
  ...
  return (
    <div>
      ...
      <div className="chem-container">
        ...
        >
          {/* <div className="chem-form-group">
            <Form.Item label="วันที่บันทึกข้อมูล" name="date">
              <DatePicker defaultValue={dayjs()} format="DD/MM/YYYY" className="chem-full-width" />
            </Form.Item>

            <Form.Item label="เวลาที่บันทึกข้อมูล" name="time">
              <TimePicker defaultValue={dayjs()} format={"HH:mm"} className="chem-full-width" />
            </Form.Item>
          </div> */}
          <div className="chem-form-group">
            <Form.Item label="วันที่บันทึกข้อมูล" name="date" rules={[{ required: true, message: 'กรุณาเลือกวันที่' }]}>
              <DatePicker format="DD/MM/YYYY" className="chem-full-width" />
            </Form.Item>
            <Form.Item label="เวลาที่บันทึกข้อมูล" name="time" rules={[{ required: true, message: 'กรุณากรอกเวลา' }]}>
              <TimePicker format="HH:mm" className="chem-full-width" />
            </Form.Item>
          </div>
          ...
          <Form.Item className="chem-form-actions" >
            <Button className="chem-cancel" htmlType="button" onClick={handleCancelClick} >
              ยกเลิก
            </Button>
            <Button htmlType="reset" className="chem-reset" onClick={handleClear} >
              รีเซ็ต
            </Button>
            <Button type="primary" htmlType="submit" className="chem-submit">
              บันทึก
            </Button>
          </Form.Item>
        </Form>
...
export default ChemicalWasteForm;
