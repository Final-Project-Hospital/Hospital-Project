...
import './generalWaste.css';
import { GeneralcenterInterface } from '../../../../../interface/Igarbage/IgeneralWaste';
import { createGeneral, GetfirstGeneral } from '../../../../../services/garbageServices/generalWaste';
...
const { Option } = Select;
const GeneralWasteForm: React.FC<Props> = ({ onCancel, onSuccess }) => {
  ...
  const GetfirstrowGeneral = async () => {
    try {
      const responfirstHAS = await GetfirstGeneral();
  ...
  const handleFinish = async (values: any) => {
    try {
      ...
      const payload: GeneralcenterInterface = {
        ...
        >
          <div className="gen-form-group">
            <Form.Item label="วันที่บันทึกข้อมูล" name="date">
              <DatePicker defaultValue={dayjs()} format="DD/MM/YYYY" className="gen-full-width" />
            </Form.Item>

            <Form.Item label="เวลาที่บันทึกข้อมูล" name="time">
              <TimePicker defaultValue={dayjs()} format={"HH:mm"} className="gen-full-width" />
            </Form.Item>
          </div>
          ...
          <Form.Item className="gen-form-actions" >
            <Button className="gen-cancel" htmlType="button" onClick={handleCancelClick} >
              ยกเลิก
            </Button>
            <Button htmlType="reset" className="gen-reset" onClick={handleClear} >
              รีเซ็ต
            </Button>
            <Button type="primary" htmlType="submit" className="gen-submit">
              บันทึก
            </Button>
          </Form.Item>
        </Form>
...
export default GeneralWasteForm;
