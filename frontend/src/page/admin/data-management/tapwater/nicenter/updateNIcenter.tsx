...
import { UpdateOrCreateNI, DeleteNI } from '../../../../../services/tapwaterServices/ni';
...
interface UpdateNICentralFormProps {
    initialValues: any; // รับค่า before/after 2 record
    onSuccess?: () => void;
    onCancel: () => void;
}

const UpdateNICentralForm: React.FC<UpdateNICentralFormProps> = ({
    initialValues,
    onSuccess,
    onCancel
}) => {
    ...
    const renderCustomTreatmentLabel = (text: string) => (
        <>
            ค่า Nitrate บริเวณบ่อพักน้ำทิ้ง
    ...
    const handleFinish = async (values: any) => {
        try {
            ...
            if (values.beforeAfterTreatmentID === 3) {
                ...
                const res = await UpdateOrCreateNI(beforePayload);
                ...
                await UpdateOrCreateNI(afterPayload);

            } else if (values.beforeAfterTreatmentID === 1) {
                await UpdateOrCreateNI({
                    ...basePayload,
                    ID: initialValues[0]?.ID ?? null,
            ...
            console.error("Error updating NI:", error?.response?.data || error);
...
export default UpdateNICentralForm;