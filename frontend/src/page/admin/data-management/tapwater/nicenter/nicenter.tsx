...
import './nicenter.css';
import { NIcenterInterface } from '../../../../../interface/Itapwater/Ini';
...
import { GetfirstNI, createNI } from '../../../../../services/tapwaterServices/ni';
...
const { Option } = Select;
const NICentralForm: React.FC<Props> = ({ onCancel, onSuccess }) => {
    ...
    const renderCustomTreatmentLabel = (text: string) => {
        ...
        return (
            <>
                ค่า Nitrate บริเวณบ่อพักน้ำทิ้ง{colored}เข้าระบบบำบัด
            </>
        );
    };
    ...
    const GetfirstrowNI = async () => {
        try {
            const responfirstNI = await GetfirstNI();
            if (responfirstNI.status === 200) {
                const data = responfirstNI.data;
                ...
                message.error("ไม่สามารถดึงข้อมูลการนัดหมายได้ สถานะ: " + responfirstNI.status);
            }
        } catch (error) {
    ...
    const handleFinish = async (values: any) => {
        ...
        if (selectedTreatmentID === 3) {
            ...
            const payloadBefore: NIcenterInterface = {
            ...
            const payloadAfter: NIcenterInterface = {
                Date: combinedDateTime.toISOString(),
            ...
            const res1 = await createNI(payloadBefore);
            const res2 = await createNI(payloadAfter);

            if ((res1 as any)?.status === 201 && (res2 as any)?.status === 201) {
                messageApi.success('บันทึกข้อมูล Nitrate ก่อนและหลังบำบัดสำเร็จ');
                form.resetFields();
            ...
            const niData: NIcenterInterface = {
                Date: combinedDateTime.toISOString(),
                Data: values.data,
                Note: values.note,
            ...
            const response = await createNI(niData);
            console.log(niData);
            if (response.status === 201) {
                messageApi.open({
                    type: 'success',
                    content: 'การบันทึกข้อมูล Nitrate สำเร็จ',
                });
                form.resetFields();
                setIsOtherunitSelected(false);
                setUseCustomStandard(false);
                ...
                >
                    <div className="ni-form-group">
                        <Form.Item label="วันที่บันทึกข้อมูล" name="date">
                            <DatePicker defaultValue={dayjs()} format="DD/MM/YYYY" className="ni-full-width" />
                        </Form.Item>

                        <Form.Item label="เวลาที่บันทึกข้อมูล" name="time">
                            <TimePicker defaultValue={dayjs()} format={"HH:mm"} className="ni-full-width" />
                        </Form.Item>
                    </div>

                    <div className="ni-form-group">
                        <div className="ni-from-mini">
                            <Form.Item
                                label="หน่วยที่วัด"
                                required
                            >
                    ...
                    <Form.Item className="ni-form-actions" >
                        <Button className="ni-cancel" htmlType="button" onClick={handleCancelClick} >
                            ยกเลิก
                        </Button>
                        <Button htmlType="reset" className="ni-reset" onClick={handleClear} >
                            รีเซ็ต
                        </Button>
                        <Button type="primary" htmlType="submit" className="ni-submit">
                            บันทึก
                        </Button>
                    </Form.Item>
                </Form>
