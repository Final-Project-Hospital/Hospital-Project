...
import { CreateNi, GetfirstNi } from "../../../../../services/tapwater";
...
import { CreateNiInterface } from "../../../../../interface/ITapwater";
...
const TKNCenterForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const GetfirstrowTKN = async () => {
      try {
        const responfirstTKN = await GetfirstNi();
  ...
  const handleFinish = async (values: any) => {
    try{
        ...
        if (selectedTreatmentID === 3) {
          ...
          const payloadBefore: CreateNiInterface = {
          ...
          const payloadAfter: CreateNiInterface = {
              Date: combinedDate.toISOString(),
          ...
          const payload: CreateNiInterface = {
              Date: combinedDate.toISOString(),
              Data: values.data,
          ...
          const res = await CreateNi(payload);
    ...
    <div>
        ...
        >
            ...
            <Button className="ni-cancel" htmlType="button">
                ยกเลิก
            </Button>
            <Button htmlType="reset" className="ni-reset" onClick={handleCancel}>
                รีเซ็ต
            </Button>
