...
import { createNI, GetfirstNI } from '../../../../../services/tapwaterServices/ni';
...
const NICentralForm: React.FC<Props> = ({ onCancel, onSuccess }) => {
    ...
    const renderCustomTreatmentLabel = (text: string) => {
        ...
        return (
            <>
                ค่า NI บริเวณบ่อพักน้ำทิ้ง{colored}เข้าระบบบำบัด
            </>
        );
    };
    ...
    const handleFinish = async (values: any) => {
        ...
        if (selectedTreatmentID === 3) {
            ...
            if ((res1 as any)?.status === 201 && (res2 as any)?.status === 201) {
                messageApi.success('บันทึกข้อมูล Ni ก่อนและหลังบำบัดสำเร็จ');
                form.resetFields();
                GetfirstrowNI();
                fetchInitialData();
                await delay(500);
            ...
            if (response.status === 201) {
                messageApi.open({
                    type: 'success',
                    content: 'การบันทึกข้อมูล NI สำเร็จ',
                });
                form.resetFields();
                GetfirstrowNI();
                fetchInitialData();
