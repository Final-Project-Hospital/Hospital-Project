...
import './TCODcenter.css';
import { TCODcenterInterface } from '../../../../../interface/Itapwater/Itcod';
import { createTCOD, GetfirstTCOD } from '../../../../../services/tapwaterServices/tcod';
...
const { Option } = Select;
const TCODCentralForm: React.FC<Props> = ({ onCancel, onSuccess }) => {
    ...
    const renderCustomTreatmentLabel = (text: string) => {
        ...
        return (
            <>
                ค่า TCOD บริเวณบ่อพักน้ำทิ้ง{colored}เข้าระบบบำบัด
            </>
        );
    };
    ...
    const GetfirstrowTCOD = async () => {
        try {
            const responfirstTCOD = await GetfirstTCOD();
            if (responfirstTCOD.status === 200) {
                const data = responfirstTCOD.data;
                ...
                message.error("ไม่สามารถดึงข้อมูลการนัดหมายได้ สถานะ: " + responfirstTCOD.status);
            }
        } catch (error) {
    ...
    const handleFinish = async (values: any) => {
        ...
        if (selectedTreatmentID === 3) {
            ...
            const payloadBefore: TCODcenterInterface = {
            ...
            const payloadAfter: TCODcenterInterface = {
                Date: combinedDateTime.toISOString(),
            ...
            const res1 = await createTCOD(payloadBefore);
            const res2 = await createTCOD(payloadAfter);
            ...
            const tcodData: TCODcenterInterface = {
                Date: combinedDateTime.toISOString(),
                Data: values.data,
                Note: values.note,
            ...
            const response = await createTCOD(tcodData);
            console.log(tcodData);
            if (response.status === 201) {
                messageApi.open({
                    type: 'success',
                    content: 'การบันทึกข้อมูล TCOD สำเร็จ',
                });
                form.resetFields();
                setIsOtherunitSelected(false);
                setUseCustomStandard(false);
    ...
    return (
        <div>
            ...
            <div className="tcod-container">
                ...
                >
                    <div className="tcod-form-group">
                        <Form.Item label="วันที่บันทึกข้อมูล" name="date">
                            <DatePicker defaultValue={dayjs()} format="DD/MM/YYYY" className="tcod-full-width" />
                        </Form.Item>

                        <Form.Item label="เวลาที่บันทึกข้อมูล" name="time">
                            <TimePicker defaultValue={dayjs()} format={"HH:mm"} className="tcod-full-width" />
                        </Form.Item>
                    </div>

                    <div className="tcod-form-group">
                        <div className="tcod-from-mini">
                            <Form.Item
                                label="หน่วยที่วัด"
                                required
                            >
                        ...
                        <div className="tcod-from-mini">
                            ...
                            <div style={{ position: 'relative', top: '-15px' }}>
                                ...
                                {standardType === 'range' && useCustomStandard && (
                                    <div className="tcod-fornt-small">
                                        <Form.Item
                                            label="ค่าต่ำสุด (Min)"
                                            name="customMin"
                    ...
                    <Form.Item className="tcod-form-actions" >
                        <Button className="tcod-cancel" htmlType="button" onClick={handleCancelClick} >
                            ยกเลิก
                        </Button>
                        <Button htmlType="reset" className="tcod-reset" onClick={handleClear} >
                            รีเซ็ต
                        </Button>
                        <Button type="primary" htmlType="submit" className="tcod-submit">
                            บันทึก
                        </Button>
                    </Form.Item>
                </Form>
...
export default TCODCentralForm;
