...
import './TTCBcenter.css';
import { TTCBcenterInterface } from '../../../../../interface/Itapwater/Ittcb';
import { createTTCB, GetfirstTTCB } from '../../../../../services/tapwaterServices/ttcb';
...
const { Option } = Select;
const TTCBCentralForm: React.FC<Props> = ({ onCancel, onSuccess }) => {
    ...
    const renderCustomTreatmentLabel = (text: string) => {
        ...
        return (
            <>
                ค่า TTCB บริเวณบ่อพักน้ำทิ้ง{colored}เข้าระบบบำบัด
            </>
        );
    };
    ...
    const GetfirstrowTTCB = async () => {
        try {
            const responfirstTTCB = await GetfirstTTCB();
            if (responfirstTTCB.status === 200) {
                const data = responfirstTTCB.data;
                ...
                message.error("ไม่สามารถดึงข้อมูลการนัดหมายได้ สถานะ: " + responfirstTTCB.status);
            }
        } catch (error) {
    ...
    const handleFinish = async (values: any) => {
        ...
        if (selectedTreatmentID === 3) {
            ...
            const payloadBefore: TTCBcenterInterface = {
            ...
            const payloadAfter: TTCBcenterInterface = {
                Date: combinedDateTime.toISOString(),
            ...
            const res1 = await createTTCB(payloadBefore);
            const res2 = await createTTCB(payloadAfter);
            ...
            const ttcbData: TTCBcenterInterface = {
                Date: combinedDateTime.toISOString(),
                Data: values.data,
                Note: values.note,
            ...
            const response = await createTTCB(ttcbData);
            console.log(ttcbData);
            if (response.status === 201) {
                messageApi.open({
                    type: 'success',
                    content: 'การบันทึกข้อมูล TTCB สำเร็จ',
                });
                form.resetFields();
                GetfirstrowTTCB();
                fetchInitialData();
    ...
    return (
        <div>
            ...
            <div className="ttcb-container">
                ...
                >
                    <div className="ttcb-form-group">
                        <Form.Item label="วันที่บันทึกข้อมูล" name="date">
                            <DatePicker defaultValue={dayjs()} format="DD/MM/YYYY" className="ttcb-full-width" />
                        </Form.Item>

                        <Form.Item label="เวลาที่บันทึกข้อมูล" name="time">
                            <TimePicker defaultValue={dayjs()} format={"HH:mm"} className="ttcb-full-width" />
                        </Form.Item>
                    </div>

                    <div className="ttcb-form-group">
                        <div className="ttcb-from-mini">
                            <Form.Item
                                label="หน่วยที่วัด"
                                required
                            >
                        ...
                        <div className="ttcb-from-mini">
                            ...
                            <div style={{ position: 'relative', top: '-15px' }}>
                                ...
                                {standardType === 'range' && useCustomStandard && (
                                    <div className="ttcb-fornt-small">
                                        <Form.Item
                                            label="ค่าต่ำสุด (Min)"
                                            name="customMin"
                    ...
                    <Form.Item className="ttcb-form-actions" >
                        <Button className="ttcb-cancel" htmlType="button" onClick={handleCancelClick} >
                            ยกเลิก
                        </Button>
                        <Button htmlType="reset" className="ttcb-reset" onClick={handleClear} >
                            รีเซ็ต
                        </Button>
                        <Button type="primary" htmlType="submit" className="ttcb-submit">
                            บันทึก
                        </Button>
                    </Form.Item>
                </Form>
...
export default TTCBCentralForm;
