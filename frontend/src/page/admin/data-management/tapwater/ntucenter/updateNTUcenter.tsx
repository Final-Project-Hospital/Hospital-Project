...
import { UpdateOrCreateNTU, DeleteNTU } from '../../../../../services/tapwaterServices/ntu';
...
interface UpdateNTUCentralFormProps {
    initialValues: any; // รับค่า before/after 2 record
    onSuccess?: () => void;
    onCancel: () => void;
}

const UpdateNTUCentralForm: React.FC<UpdateNTUCentralFormProps> = ({
    initialValues,
    onSuccess,
    onCancel
}) => {
    ...
    const handleFinish = async (values: any) => {
        try {
            ...
            if (values.beforeAfterTreatmentID === 3) {
                ...
                const res = await UpdateOrCreateNTU(beforePayload);
                ...
                await UpdateOrCreateNTU(afterPayload);

            } else if (values.beforeAfterTreatmentID === 1) {
                await UpdateOrCreateNTU({
                    ...basePayload,
                    ID: initialValues[0]?.ID ?? null,
            ...
            console.error("Error updating NTU:", error?.response?.data || error);
...
export default UpdateNTUCentralForm;