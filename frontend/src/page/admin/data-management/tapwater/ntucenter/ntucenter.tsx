...
import './ntucenter.css';
import { NTUcenterInterface } from '../../../../../interface/Itapwater/Intu';
...
import { GetfirstNTU, createNTU } from '../../../../../services/tapwaterServices/ntu';
...
const { Option } = Select;
const NTUCentralForm: React.FC<Props> = ({ onCancel, onSuccess }) => {
    ...
    const renderCustomTreatmentLabel = (text: string) => {
        ...
        return (
            <>
                ค่า NTU บริเวณบ่อพักน้ำทิ้ง{colored}เข้าระบบบำบัด
            </>
        );
    };
    ...
    const GetfirstrowNTU = async () => {
        try {
            const responfirstNTU = await GetfirstNTU();
            if (responfirstNTU.status === 200) {
                const data = responfirstNTU.data;
                ...
                message.error("ไม่สามารถดึงข้อมูลการนัดหมายได้ สถานะ: " + responfirstNTU.status);
            }
        } catch (error) {
    ...
    const handleFinish = async (values: any) => {
        ...
        if (selectedTreatmentID === 3) {
            ...
            const payloadBefore: NTUcenterInterface = {
            ...
            const payloadAfter: NTUcenterInterface = {
                Date: combinedDateTime.toISOString(),
            ...
            const res1 = await createNTU(payloadBefore);
            const res2 = await createNTU(payloadAfter);

            if ((res1 as any)?.status === 201 && (res2 as any)?.status === 201) {
                messageApi.success('บันทึกข้อมูล NTU ก่อนและหลังบำบัดสำเร็จ');
                form.resetFields();
            ...
            const ntuData: NTUcenterInterface = {
                Date: combinedDateTime.toISOString(),
                Data: values.data,
                Note: values.note,
            ...
            const response = await createNTU(ntuData);
            console.log(ntuData);
            if (response.status === 201) {
                messageApi.open({
                    type: 'success',
                    content: 'การบันทึกข้อมูล NTU สำเร็จ',
                });
                form.resetFields();
                setIsOtherunitSelected(false);
                setUseCustomStandard(false);
    ...
    return (
        <div>
            ...
            <div className="ntu-container">
                ...
                >
                    <div className="ntu-form-group">
                        <Form.Item label="วันที่บันทึกข้อมูล" name="date">
                            <DatePicker defaultValue={dayjs()} format="DD/MM/YYYY" className="ntu-full-width" />
                        </Form.Item>

                        <Form.Item label="เวลาที่บันทึกข้อมูล" name="time">
                            <TimePicker defaultValue={dayjs()} format={"HH:mm"} className="ntu-full-width" />
                        </Form.Item>
                    </div>

                    <div className="ntu-form-group">
                        <div className="ntu-from-mini">
                            <Form.Item
                                label="หน่วยที่วัด"
                                required
                            >
                    ...
                    <Form.Item className="ntu-form-actions" >
                        <Button className="ntu-cancel" htmlType="button" onClick={handleCancelClick} >
                            ยกเลิก
                        </Button>
                        <Button htmlType="reset" className="ntu-reset" onClick={handleClear} >
                            รีเซ็ต
                        </Button>
                        <Button type="primary" htmlType="submit" className="ntu-submit">
                            บันทึก
                        </Button>
                    </Form.Item>
                </Form>
