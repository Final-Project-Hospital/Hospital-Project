...
import { CreateNTU, GetfirstNTU } from "../../../../../services/tapwater";
...
import { CreateNTUInterface } from "../../../../../interface/ITapwater";
...
const TKNCenterForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const GetfirstrowTKN = async () => {
      try {
        const responfirstTKN = await GetfirstNTU();
  ...
  const handleFinish = async (values: any) => {
    try{
        ...
        if (selectedTreatmentID === 3) {
          ...
          const payloadBefore: CreateNTUInterface = {
          ...
          const payloadAfter: CreateNTUInterface = {
              Date: combinedDate.toISOString(),
          ...
          const payload: CreateNTUInterface = {
              Date: combinedDate.toISOString(),
              Data: values.data,
          ...
          const res = await CreateNTU(payload);
    ...
    <div>
        ...
        >
            ...
            <Button className="ntu-cancel" htmlType="button">
                ยกเลิก
            </Button>
            <Button htmlType="reset" className="ntu-reset" onClick={handleCancel}>
                รีเซ็ต
            </Button>
