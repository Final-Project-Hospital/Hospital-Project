...
import { UpdateOrCreateIRON, DeleteIRON } from '../../../../../services/tapwaterServices/iron';
...
interface UpdateIRONCentralFormProps {
    initialValues: any; // รับค่า before/after 2 record
    onSuccess?: () => void;
    onCancel: () => void;
}

const UpdateIRONCentralForm: React.FC<UpdateIRONCentralFormProps> = ({
    initialValues,
    onSuccess,
    onCancel
}) => {
    ...
    const handleFinish = async (values: any) => {
        try {
            ...
            if (values.beforeAfterTreatmentID === 3) {
                ...
                const res = await UpdateOrCreateIRON(beforePayload);
                ...
                await UpdateOrCreateIRON(afterPayload);

            } else if (values.beforeAfterTreatmentID === 1) {
                await UpdateOrCreateIRON({
                    ...basePayload,
                    ID: initialValues[0]?.ID ?? null,
            ...
            console.error("Error updating IRON:", error?.response?.data || error);
...
export default UpdateIRONCentralForm;