...
import { CreateIron, GetfirstIron } from "../../../../../services/tapwater";
...
import { CreateIronInterface } from "../../../../../interface/ITapwater";
...
const TKNCenterForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const GetfirstrowTKN = async () => {
      try {
        const responfirstTKN = await GetfirstIron();
  ...
  const handleFinish = async (values: any) => {
    try{
        ...
        if (selectedTreatmentID === 3) {
          ...
          const payloadBefore: CreateIronInterface = {
          ...
          const payloadAfter: CreateIronInterface = {
              Date: combinedDate.toISOString(),
          ...
          const payload: CreateIronInterface = {
              Date: combinedDate.toISOString(),
              Data: values.data,
          ...
          const res = await CreateIron(payload);
    ...
    <div>
        ...
        >
            ...
            <Form.Item label="วันที่บันทึกข้อมูล" name="date">
                <DatePicker defaultValue={dayjs()} format="DD/MM/YYYY" className="ir-full-width" />
            </Form.Item>

            <Form.Item label="เวลาที่บันทึกข้อมูล" name="time">
                <TimePicker defaultValue={dayjs()} format={"HH:mm"} className="ir-full-width" />
            </Form.Item>
            </div>

            <div className="ir-form-group">
            <div className="ir-form-mini">
                <Form.Item
                label="หน่วยที่วัด"
                required
                >
            ...
            <Form.Item className="ir-form-actions">
            <Button className="ir-cancel" htmlType="button">
                ยกเลิก
            </Button>
            <Button htmlType="reset" className="ir-reset" onClick={handleCancel}>
                รีเซ็ต
            </Button>
            <Button type="primary" htmlType="submit" className="ir-submit">
                บันทึก
            </Button>
            </Form.Item>
        </Form>
