...
import './alcenter.css';
import { ALcenterInterface } from '../../../../../interface/Itapwater/Ial';
import { createAL, GetfirstAL } from '../../../../../services/tapwaterServices/al';
...
const { Option } = Select;
const ALCentralForm: React.FC<Props> = ({ onCancel, onSuccess }) => {
    ...
    const renderCustomTreatmentLabel = (text: string) => {
        ...
        return (
            <>
                ค่า AL บริเวณบ่อพักน้ำทิ้ง{colored}เข้าระบบบำบัด
            </>
        );
    };
    ...
    const GetfirstrowAL = async () => {
        try {
            const responfirstAL = await GetfirstAL();
    ...
    const handleFinish = async (values: any) => {
        ...
        if (selectedTreatmentID === 3) {
            ...
            const payloadBefore: ALcenterInterface = {
            ...
            const payloadAfter: ALcenterInterface = {
                Date: combinedDateTime.toISOString(),
            ...
            const alData: ALcenterInterface = {
                Date: combinedDateTime.toISOString(),
                Data: values.data,
                Note: values.note,
                ...
                >
                    <div className="al-form-group">
                        <Form.Item label="วันที่บันทึกข้อมูล" name="date">
                            <DatePicker defaultValue={dayjs()} format="DD/MM/YYYY" className="al-full-width" />
                        </Form.Item>

                        <Form.Item label="เวลาที่บันทึกข้อมูล" name="time">
                            <TimePicker defaultValue={dayjs()} format={"HH:mm"} className="al-full-width" />
                        </Form.Item>
                    </div>

                    <div className="al-form-group">
                        <div className="al-from-mini">
                            <Form.Item
                                label="หน่วยที่วัด"
                                required
                            >
                    ...
                    <Form.Item className="al-form-actions" >
                        <Button className="al-cancel" htmlType="button" onClick={handleCancelClick} >
                            ยกเลิก
                        </Button>
                        <Button htmlType="reset" className="al-reset" onClick={handleClear} >
                            รีเซ็ต
                        </Button>
                        <Button type="primary" htmlType="submit" className="al-submit">
                            บันทึก
                        </Button>
                    </Form.Item>
                </Form>
