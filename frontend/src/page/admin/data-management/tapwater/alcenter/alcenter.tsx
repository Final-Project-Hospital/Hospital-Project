...
import { CreateAL, GetfirstAL } from "../../../../../services/tapwater";
...
import { CreateALInterface } from "../../../../../interface/ITapwater";
...
const TKNCenterForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const GetfirstrowTKN = async () => {
      try {
        const responfirstTKN = await GetfirstAL();
  ...
  const handleFinish = async (values: any) => {
    try{
        ...
        if (selectedTreatmentID === 3) {
          ...
          const payloadBefore: CreateALInterface = {
          ...
          const payloadAfter: CreateALInterface = {
              Date: combinedDate.toISOString(),
          ...
          const payload: CreateALInterface = {
              Date: combinedDate.toISOString(),
              Data: values.data,
          ...
          const res = await CreateAL(payload);
    ...
    <div>
        ...
        >
            ...
            <Button className="al-cancel" htmlType="button">
                ยกเลิก
            </Button>
            <Button htmlType="reset" className="al-reset" onClick={handleCancel}>
                รีเซ็ต
            </Button>
