...
import { UpdateOrCreateAL, DeleteAL } from '../../../../../services/tapwaterServices/al';
...
interface UpdateALCentralFormProps {
    initialValues: any; // รับค่า before/after 2 record
    onSuccess?: () => void;
    onCancel: () => void;
}

const UpdateALCentralForm: React.FC<UpdateALCentralFormProps> = ({
    initialValues,
    onSuccess,
    onCancel
}) => {
    ...
    const handleFinish = async (values: any) => {
        try {
            ...
            if (values.beforeAfterTreatmentID === 3) {
                ...
                const res = await UpdateOrCreateAL(beforePayload);
                ...
                await UpdateOrCreateAL(afterPayload);

            } else if (values.beforeAfterTreatmentID === 1) {
                await UpdateOrCreateAL({
                    ...basePayload,
                    ID: initialValues[0]?.ID ?? null,
            ...
            console.error("Error updating AL:", error?.response?.data || error);
...
export default UpdateALCentralForm;