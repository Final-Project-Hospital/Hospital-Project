...
import { CreateMn, GetfirstMn } from "../../../../../services/tapwater";
...
import { CreateMnInterface } from "../../../../../interface/ITapwater";
...
const TKNCenterForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const GetfirstrowTKN = async () => {
      try {
        const responfirstTKN = await GetfirstMn();
  ...
  const handleFinish = async (values: any) => {
    try{
        ...
        if (selectedTreatmentID === 3) {
          ...
          const payloadBefore: CreateMnInterface = {
          ...
          const payloadAfter: CreateMnInterface = {
              Date: combinedDate.toISOString(),
          ...
          const payload: CreateTKNInterface = {
              Date: combinedDate.toISOString(),
              Data: values.data,
          ...
          const res = await CreateMn(payload);
    ...
    <div>
        ...
        >
            ...
            <Button className="mn-cancel" htmlType="button">
                ยกเลิก
            </Button>
            <Button htmlType="reset" className="mn-reset" onClick={handleCancel}>
                รีเซ็ต
            </Button>
