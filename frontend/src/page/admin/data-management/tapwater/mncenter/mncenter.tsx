...
import './mncenter.css';
import { MNcenterInterface } from '../../../../../interface/Itapwater/Imn';
...
import { GetfirstMN, createMN } from '../../../../../services/tapwaterServices/mn';
...
const { Option } = Select;
const MNCentralForm: React.FC<Props> = ({ onCancel, onSuccess }) => {
    ...
    const renderCustomTreatmentLabel = (text: string) => {
        ...
        return (
            <>
                ค่า Mn บริเวณบ่อพักน้ำทิ้ง{colored}เข้าระบบบำบัด
            </>
        );
    };
    ...
    const GetfirstrowMN = async () => {
        try {
            const responfirstMN = await GetfirstMN();
            if (responfirstMN.status === 200) {
                const data = responfirstMN.data;
                ...
                message.error("ไม่สามารถดึงข้อมูลการนัดหมายได้ สถานะ: " + responfirstMN.status);
            }
        } catch (error) {
    ...
    const handleFinish = async (values: any) => {
        ...
        if (selectedTreatmentID === 3) {
            ...
            const payloadBefore: MNcenterInterface = {
            ...
            const payloadAfter: MNcenterInterface = {
                Date: combinedDateTime.toISOString(),
            ...
            const res1 = await createMN(payloadBefore);
            const res2 = await createMN(payloadAfter);

            if ((res1 as any)?.status === 201 && (res2 as any)?.status === 201) {
                messageApi.success('บันทึกข้อมูล Mn ก่อนและหลังบำบัดสำเร็จ');
                form.resetFields();
            ...
            const mnData: MNcenterInterface = {
                Date: combinedDateTime.toISOString(),
                Data: values.data,
                Note: values.note,
            ...
            const response = await createMN(mnData);
            console.log(mnData);
            if (response.status === 201) {
                messageApi.open({
                    type: 'success',
                    content: 'การบันทึกข้อมูล Mn สำเร็จ',
                });
                form.resetFields();
                setIsOtherunitSelected(false);
                setUseCustomStandard(false);
                ...
                >
                    <div className="mn-form-group">
                        <Form.Item label="วันที่บันทึกข้อมูล" name="date">
                            <DatePicker defaultValue={dayjs()} format="DD/MM/YYYY" className="mn-full-width" />
                        </Form.Item>

                        <Form.Item label="เวลาที่บันทึกข้อมูล" name="time">
                            <TimePicker defaultValue={dayjs()} format={"HH:mm"} className="mn-full-width" />
                        </Form.Item>
                    </div>

                    <div className="mn-form-group">
                        <div className="mn-from-mini">
                            <Form.Item
                                label="หน่วยที่วัด"
                                required
                            >
                    ...
                    <Form.Item className="mn-form-actions" >
                        <Button className="mn-cancel" htmlType="button" onClick={handleCancelClick} >
                            ยกเลิก
                        </Button>
                        <Button htmlType="reset" className="mn-reset" onClick={handleClear} >
                            รีเซ็ต
                        </Button>
                        <Button type="primary" htmlType="submit" className="mn-submit">
                            บันทึก
                        </Button>
                    </Form.Item>
                </Form>
