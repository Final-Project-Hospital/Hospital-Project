...
import { UpdateOrCreateMN, DeleteMN } from '../../../../../services/tapwaterServices/mn';
...
interface UpdateMNCentralFormProps {
    initialValues: any; // รับค่า before/after 2 record
    onSuccess?: () => void;
    onCancel: () => void;
}

const UpdateMNCentralForm: React.FC<UpdateMNCentralFormProps> = ({
    initialValues,
    onSuccess,
    onCancel
}) => {
    ...
    const handleFinish = async (values: any) => {
        try {
            ...
            if (values.beforeAfterTreatmentID === 3) {
                ...
                const res = await UpdateOrCreateMN(beforePayload);
                ...
                await UpdateOrCreateMN(afterPayload);

            } else if (values.beforeAfterTreatmentID === 1) {
                await UpdateOrCreateMN({
                    ...basePayload,
                    ID: initialValues[0]?.ID ?? null,
            ...
            console.error("Error updating MN:", error?.response?.data || error);
...
export default UpdateMNCentralForm;