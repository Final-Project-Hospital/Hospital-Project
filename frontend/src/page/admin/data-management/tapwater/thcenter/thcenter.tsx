...
import './THcenter.css';
import { THcenterInterface } from '../../../../../interface/Itapwater/Ith';
import { createTH, GetfirstTH } from '../../../../../services/tapwaterServices/th';
...
const { Option } = Select;
const THCentralForm: React.FC<Props> = ({ onCancel, onSuccess }) => {
    ...
    const renderCustomTreatmentLabel = (text: string) => {
        ...
        return (
            <>
                ค่า TH บริเวณบ่อพักน้ำทิ้ง{colored}เข้าระบบบำบัด
            </>
        );
    };
    ...
    const GetfirstrowTH = async () => {
        try {
            const responfirstTH = await GetfirstTH();
            if (responfirstTH.status === 200) {
                const data = responfirstTH.data;
                ...
                message.error("ไม่สามารถดึงข้อมูลการนัดหมายได้ สถานะ: " + responfirstTH.status);
            }
        } catch (error) {
    ...
    const handleFinish = async (values: any) => {
        ...
        if (selectedTreatmentID === 3) {
            ...
            const payloadBefore: THcenterInterface = {
            ...
            const payloadAfter: THcenterInterface = {
                Date: combinedDateTime.toISOString(),
            ...
            const res1 = await createTH(payloadBefore);
            const res2 = await createTH(payloadAfter);

            if ((res1 as any)?.status === 201 && (res2 as any)?.status === 201) {
                messageApi.success('บันทึกข้อมูล TH ก่อนและหลังบำบัดสำเร็จ');
                form.resetFields();
            ...
            const thData: THcenterInterface = {
                Date: combinedDateTime.toISOString(),
                Data: values.data,
                Note: values.note,
            ...
            const response = await createTH(thData);
            console.log(thData);
            if (response.status === 201) {
                messageApi.open({
                    type: 'success',
                    content: 'การบันทึกข้อมูล TH สำเร็จ',
                });
                form.resetFields();
                setIsOtherunitSelected(false);
                setUseCustomStandard(false);
                ...
                >
                    <div className="th-form-group">
                        <Form.Item label="วันที่บันทึกข้อมูล" name="date">
                            <DatePicker defaultValue={dayjs()} format="DD/MM/YYYY" className="th-full-width" />
                        </Form.Item>

                        <Form.Item label="เวลาที่บันทึกข้อมูล" name="time">
                            <TimePicker defaultValue={dayjs()} format={"HH:mm"} className="th-full-width" />
                        </Form.Item>
                    </div>

                    <div className="th-form-group">
                        <div className="th-from-mini">
                            <Form.Item
                                label="หน่วยที่วัด"
                                required
                            >
                    ...
                    <Form.Item className="th-form-actions" >
                        <Button className="th-cancel" htmlType="button" onClick={handleCancelClick} >
                            ยกเลิก
                        </Button>
                        <Button htmlType="reset" className="th-reset" onClick={handleClear} >
                            รีเซ็ต
                        </Button>
                        <Button type="primary" htmlType="submit" className="th-submit">
                            บันทึก
                        </Button>
                    </Form.Item>
                </Form>
