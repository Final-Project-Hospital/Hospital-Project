...
import { CreateTH, GetfirstTH } from "../../../../../services/tapwater";
...
import { CreateTHInterface } from "../../../../../interface/ITapwater";
...
const TKNCenterForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const GetfirstrowTKN = async () => {
      try {
        const responfirstTKN = await GetfirstTH();
  ...
  const handleFinish = async (values: any) => {
    try{
        ...
        if (selectedTreatmentID === 3) {
          ...
          const payloadBefore: CreateTHInterface = {
          ...
          const payloadAfter: CreateTHInterface = {
              Date: combinedDate.toISOString(),
          ...
          const payload: CreateTHInterface = {
              Date: combinedDate.toISOString(),
              Data: values.data,
          ...
          const res = await CreateTH(payload);
    ...
    <div>
        ...
        >
            ...
            <Button className="th-cancel" htmlType="button">
                ยกเลิก
            </Button>
            <Button htmlType="reset" className="th-reset" onClick={handleCancel}>
                รีเซ็ต
            </Button>
