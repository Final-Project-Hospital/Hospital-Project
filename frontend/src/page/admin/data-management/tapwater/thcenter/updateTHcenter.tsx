...
import { UpdateOrCreateTH, DeleteTH } from '../../../../../services/tapwaterServices/th';
...
interface UpdateTHCentralFormProps {
    initialValues: any; // รับค่า before/after 2 record
    onSuccess?: () => void;
    onCancel: () => void;
}

const UpdateTHCentralForm: React.FC<UpdateTHCentralFormProps> = ({
    initialValues,
    onSuccess,
    onCancel
}) => {
    ...
    const handleFinish = async (values: any) => {
        try {
            ...
            if (values.beforeAfterTreatmentID === 3) {
                ...
                const res = await UpdateOrCreateTH(beforePayload);
                ...
                await UpdateOrCreateTH(afterPayload);

            } else if (values.beforeAfterTreatmentID === 1) {
                await UpdateOrCreateTH({
                    ...basePayload,
                    ID: initialValues[0]?.ID ?? null,
            ...
            console.error("Error updating TH:", error?.response?.data || error);
...
export default UpdateTHCentralForm;