...
import { CreatePT, GetfirstPT } from "../../../../../services/tapwater";
...
import { CreatePTInterface } from "../../../../../interface/ITapwater";
...
const TKNCenterForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const GetfirstrowTKN = async () => {
      try {
        const responfirstTKN = await GetfirstPT();
  ...
  const handleFinish = async (values: any) => {
    try{
        ...
        if (selectedTreatmentID === 3) {
          ...
          const payloadBefore: CreatePTInterface = {
          ...
          const payloadAfter: CreatePTInterface = {
              Date: combinedDate.toISOString(),
          ...
          const payload: CreatePTInterface = {
              Date: combinedDate.toISOString(),
              Data: values.data,
          ...
          const res = await CreatePT(payload);
    ...
    <div>
        ...
        >
            ...
            <Button className="pt-cancel" htmlType="button">
                ยกเลิก
            </Button>
            <Button htmlType="reset" className="pt-reset" onClick={handleCancel}>
                รีเซ็ต
            </Button>
