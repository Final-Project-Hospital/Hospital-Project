...
const { Option } = Select;
const PTCentralForm: React.FC<Props> = ({ onCancel, onSuccess }) => {
    ...
    const renderCustomTreatmentLabel = (text: string) => {
        ...
        return (
            <>
                ค่า PT บริเวณบ่อพักน้ำทิ้ง{colored}เข้าระบบบำบัด
            </>
        );
    };
    ...
    const GetfirstrowPT = async () => {
        try {
            const responfirstPT = await GetfirstPT();
            if (responfirstPT.status === 200) {
                const data = responfirstPT.data;
                ...
                message.error("ไม่สามารถดึงข้อมูลการนัดหมายได้ สถานะ: " + responfirstPT.status);
            }
        } catch (error) {
    ...
    const handleFinish = async (values: any) => {
        ...
        if (selectedTreatmentID === 3) {
            ...
            const payloadBefore: PTcenterInterface = {
            ...
            const payloadAfter: PTcenterInterface = {
                Date: combinedDateTime.toISOString(),
            ...
            const res1 = await createPT(payloadBefore);
            const res2 = await createPT(payloadAfter);

            if ((res1 as any)?.status === 201 && (res2 as any)?.status === 201) {
                messageApi.success('บันทึกข้อมูล PT ก่อนและหลังบำบัดสำเร็จ');
                form.resetFields();
            ...
            const ptData: PTcenterInterface = {
                Date: combinedDateTime.toISOString(),
                Data: values.data,
                Note: values.note,
            ...
            const response = await createPT(ptData);
            console.log(ptData);
            if (response.status === 201) {
                messageApi.open({
                    type: 'success',
                    content: 'การบันทึกข้อมูล PT สำเร็จ',
                });
                form.resetFields();
                setIsOtherunitSelected(false);
                setUseCustomStandard(false);
                ...
                >
                    <div className="pt-form-group">
                        <Form.Item label="วันที่บันทึกข้อมูล" name="date">
                            <DatePicker defaultValue={dayjs()} format="DD/MM/YYYY" className="pt-full-width" />
                        </Form.Item>

                        <Form.Item label="เวลาที่บันทึกข้อมูล" name="time">
                            <TimePicker defaultValue={dayjs()} format={"HH:mm"} className="pt-full-width" />
                        </Form.Item>
                    </div>

                    <div className="pt-form-group">
                        <div className="pt-from-mini">
                            <Form.Item
                                label="หน่วยที่วัด"
                                required
                            >
                    ...
                    <Form.Item className="pt-form-actions" >
                        <Button className="pt-cancel" htmlType="button" onClick={handleCancelClick} >
                            ยกเลิก
                        </Button>
                        <Button htmlType="reset" className="pt-reset" onClick={handleClear} >
                            รีเซ็ต
                        </Button>
                        <Button type="primary" htmlType="submit" className="pt-submit">
                            บันทึก
                        </Button>
                    </Form.Item>
                </Form>
