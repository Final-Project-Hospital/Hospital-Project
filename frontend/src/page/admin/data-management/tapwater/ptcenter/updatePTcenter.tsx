...
import { UpdateOrCreatePT, DeletePT } from '../../../../../services/tapwaterServices/pt';
...
interface UpdatePTCentralFormProps {
    initialValues: any; // รับค่า before/after 2 record
    onSuccess?: () => void;
    onCancel: () => void;
}

const UpdatePTCentralForm: React.FC<UpdatePTCentralFormProps> = ({
    initialValues,
    onSuccess,
    onCancel
}) => {
    ...
    const handleFinish = async (values: any) => {
        try {
            ...
            if (values.beforeAfterTreatmentID === 3) {
                ...
                const res = await UpdateOrCreatePT(beforePayload);
                ...
                await UpdateOrCreatePT(afterPayload);

            } else if (values.beforeAfterTreatmentID === 1) {
                await UpdateOrCreatePT({
                    ...basePayload,
                    ID: initialValues[0]?.ID ?? null,
            ...
            console.error("Error updating PT:", error?.response?.data || error);
...
export default UpdatePTCentralForm;