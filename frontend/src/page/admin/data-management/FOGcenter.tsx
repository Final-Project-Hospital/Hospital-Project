...
const TDSCentralForm: React.FC = () => {
    ...
    const renderCustomTreatmentLabel = (text: string) => {
        ...
        return (
            <>
                ค่า FOG บริเวณบ่อพักน้ำทิ้ง{colored}เข้าระบบบำบัด
            </>
        );
    };
    ...
    const handleFinish = async (values: any) => {
        ...
        if (selectedTreatmentID === 3) {
            ...
            const res1 = await createFOG(payloadBefore);
                ...
                >
                    <div className="fog-form-group">
                        <Form.Item label="วันที่บันทึกข้อมูล" name="date">
                            <DatePicker defaultValue={dayjs()} format="DD/MM/YYYY" className="fog-full-width" />
                        </Form.Item>
